...
class InvoiceViewModel : ViewModel() {
    ...
    // Real-time listener for invoices, with sorting in descending order by posting date
    private val invoiceListener = object : ValueEventListener {
        override fun onDataChange(snapshot: DataSnapshot) {
            val invoices = snapshot.children.mapNotNull { it.getValue(Invoice::class.java) }
                .sortedByDescending { it.postingDate } // Sort by postingDate in descending order
            _invoiceList.value = invoices
        }
    ...
    // Fetch invoices from Firebase (manually triggered), sorted in descending order by posting date
    fun fetchInvoices() {
        viewModelScope.launch {
            database.get().addOnSuccessListener { snapshot ->
                val invoices = snapshot.children.mapNotNull { it.getValue(Invoice::class.java) }
